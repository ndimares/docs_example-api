...
const (
	...
	testPassword     = "guest"
	testDB           = "test"
	postgresDBConfig = "host=localhost port=%s user=%s password=%s dbname=%s sslmode=disable TimeZone=UTC"
)
...
func TestMain(m *testing.M) {
	...
	resource, err := pool.Run("postgres", "alpine", []string{fmt.Sprintf("POSTGRES_USER=%s", testUsername), fmt.Sprintf("POSTGRES_PASSWORD=%s", testPassword), fmt.Sprintf("POSTGRES_DB=%s", testDB)})
	if err != nil {
	...
	if err := pool.Retry(func() error {
		dsn := fmt.Sprintf(postgresDBConfig, resource.GetPort("5432/tcp"), testUsername, testPassword, testDB)

		db = psql.New(psql.Config{DSN: dsn})
