//go:generate mockgen -destination=./mocks/users_mock.go -package mocks github.com/speakeasy-api/speakeasy-test-rest-service/internal/users Store,Events
...
import (
	...
	"github.com/speakeasy-api/speakeasy-test-rest-service/internal/users/model"
	"go.uber.org/zap"
)
...
func (u *Users) FindUsers(ctx context.Context, filters []model.Filter, offset, limit int64) ([]*model.User, error) {
	...
	for i, f := range filters {
		if f.Value == "" {
			...
			log.From(ctx).Error("empty filter value provided", zap.Error(err), zap.Int("index", i))
			return nil, err
		}
		...
		default:
			...
			log.From(ctx).Error("match type not supported", zap.Error(err), zap.String("match_type", string(f.MatchType)), zap.Int("index", i))
			return nil, err
		}
		...
		default:
			...
			log.From(ctx).Error("filter field not supported", zap.Error(err), zap.String("field", string(f.Field)), zap.Int("index", i))
			return nil, err
		}
	}
