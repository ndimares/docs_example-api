...
func handleError(ctx context.Context, w http.ResponseWriter, err error) {
	log.From(ctx).Error("error occurred in request", zap.Error(err))
	...
	if err != nil {
		log.From(ctx).Error("failed to serialize error response", zap.Error(err))
		data = []byte(`{"error": "internal server error"}`)
	}

	_, err = w.Write(data)
	if err != nil {
		log.From(ctx).Error("failed to write error response", zap.Error(err))
	}
}
